 ...
 */
function MultiFileUploader(input_id, obj_file, service_action, parent_container, complete_action, file_handling)
{
    ...
    this.createStatusRow = function()
    {
        ...
        var file_row_wrapper  = $('<div />',  {'class': 'tfile_row_wrapper'});
    ...
    this.showFile = function(file_data)
    {
        ...
        if (file_name) {
            ...
            if ($.inArray( file_name.split('.').pop(), [ "png", "jpg" ] ) > -1) {
                ...
                file_link.popover({ trigger: "hover", 'content': '<img style="max-width:460px" src="'+'download.php?file='+file_name+'">', 'html' : true, 'container': 'body', 'template': pop_template });
            }
            else {
                var file_link = $('<a />', { 'href': 'download.php?file='+file_name, 'target' : '_blank' });
            }
    ...
    this.showMessage = function(type, message)
    {
        if (type == 'success') {
            that.parent_container.find('[widget=tmultifile]').css('padding-left', '20px');
            that.parent_container.children('i').attr({ 'class' : 'fa fa-check-circle-o green file-response-icon',
                                                       'title' : message });
        }
        else if (type == 'error') {
            that.parent_container.find('[widget=tmultifile]').css('padding-left', '20px');
...
function tmultifile_start( input_id, parent_container, service_action, complete_action, file_handling )
{
    $(function() {
        $('#' + input_id).change( function() {
            // iterate selected files in upload dialog
            $.each( $(this).prop('files'), function(index, upload_file) {
                var file = new  MultiFileUploader(input_id, upload_file, service_action, parent_container, complete_action, file_handling);                    
                file.initFileUpload();
            });
        });
        ...
        if (file_handling) {
            $('#'+input_id).parent().children('input[type=hidden]').each(function(index, input_hidden) {
                var file = new  MultiFileUploader(input_id, null, service_action, parent_container, complete_action, file_handling);
